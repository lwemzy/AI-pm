...
export function ReportViewer({ reportType }: ReportViewerProps) {
    const {
        prescriptions,
        processedData
    } = useData();
        ...
        switch (reportType) {
            ...
            case 'pricing':
                if (!processedData.hasPricingData || !processedData.pricingMetrics) {
                    return <div className="bg-white p-6 rounded-lg shadow"><p className="text-center text-gray-500 py-8">No pricing data available. Upload CSV with pricing columns.</p></div>;
                }
                const pm = processedData.pricingMetrics;
                const pricingByDrug: any = {};
                prescriptions.forEach(rx => {
                    if (rx.Plan_Paid_Amount == null || rx.Reimbursement_Amount == null) return;
                    const key = rx.Drug_Class;
                    if (!pricingByDrug[key]) pricingByDrug[key] = { name: key, spread: 0, count: 0, planPaid: 0 };
                    pricingByDrug[key].spread += rx.Plan_Paid_Amount - rx.Reimbursement_Amount;
                    pricingByDrug[key].planPaid += rx.Plan_Paid_Amount;
                    pricingByDrug[key].count++;
                });
                const pricingChartData = Object.values(pricingByDrug).map((d: any) => ({
                    name: d.name, avgSpread: d.spread / d.count, avgPlanPaid: d.planPaid / d.count
                }));
                return <div className="space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-white p-6 rounded-lg shadow">
                            <h3 className="text-sm font-medium text-gray-500">Total Claims</h3>
                            <p className="mt-2 text-3xl font-bold text-gray-900">${pm.totalClaimAmount.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
                        </div>
                        <div className="bg-white p-6 rounded-lg shadow">
                            <h3 className="text-sm font-medium text-gray-500">Average Spread</h3>
                            <p className="mt-2 text-3xl font-bold text-gray-900">${pm.averageSpread.toFixed(2)}</p>
                        </div>
                        <div className="bg-white p-6 rounded-lg shadow">
                            <h3 className="text-sm font-medium text-gray-500">Total Rebates</h3>
                            <p className="mt-2 text-3xl font-bold text-gray-900">${pm.totalRebates.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
                        </div>
                        <div className="bg-white p-6 rounded-lg shadow">
                            <h3 className="text-sm font-medium text-gray-500">Patient Paid</h3>
                            <p className="mt-2 text-3xl font-bold text-gray-900">${pm.totalPatientPaid.toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
                        </div>
                    </div>
                    <div className="bg-white p-6 rounded-lg shadow">
                        <h3 className="text-lg font-medium text-gray-900 mb-6">Spread by Drug Class</h3>
                        <div className="h-[400px]">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={pricingChartData}>
                                    ...
                                    <Legend />
                                    <Bar dataKey="avgSpread" name="Avg Spread" fill="#ef4444" />
                                    <Bar dataKey="avgPlanPaid" name="Avg Plan Paid" fill="#3b82f6" />
                                </BarChart>
                            </ResponsiveContainer>
                        </div>
                    </div>
            ...
            case 'pbm-comparison':
                if (!processedData.pbmMetrics || processedData.pbmMetrics.pbmList.length === 0) {
                    return <div className="bg-white p-6 rounded-lg shadow"><p className="text-center text-gray-500 py-8">No PBM data available.</p></div>;
                }
                const pbmM = processedData.pbmMetrics;
                const pbmChartData = pbmM.pbmList.map(pbm => ({
                    name: pbm,
                    claims: pbmM.claimsByPBM[pbm] || 0,
                    avgCost: pbmM.avgCostByPBM[pbm] || 0,
                    ger: pbmM.genericRateByPBM[pbm] || 0
                }));
                return <div className="space-y-6">
                    <div className="bg-white p-6 rounded-lg shadow">
                        <h3 className="text-lg font-medium text-gray-900 mb-6">PBM Comparison — Average Cost per Claim</h3>
                        <div className="h-[400px]">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={pbmChartData}>
                                    <CartesianGrid strokeDasharray="3 3" />
                                    <XAxis dataKey="name" />
                                    <YAxis tickFormatter={(v) => `$${v.toFixed(0)}`} />
                                    <Tooltip formatter={(value: number, name: string) => name === 'avgCost' ? `$${value.toFixed(2)}` : value} />
                                    <Legend />
                                    <Bar dataKey="avgCost" name="Avg Cost/Claim" fill="#3b82f6" />
                                </BarChart>
                            </ResponsiveContainer>
                        </div>
                    </div>
                    <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                        {pbmChartData.map(p => (
                            <div key={p.name} className="bg-white p-4 rounded-lg shadow">
                                ...
                                <p className="text-sm text-gray-500">{p.claims.toLocaleString()} claims</p>
                                <p className="text-sm text-gray-500">GER: {p.ger.toFixed(1)}%</p>
                            </div>
                        ))}
                    </div>
                </div>;
            case 'audit':
                if (!processedData.auditMetrics) {
                    return <div className="bg-white p-6 rounded-lg shadow"><p className="text-center text-gray-500 py-8">No audit data available.</p></div>;
                }
                const auditM = processedData.auditMetrics;
                return <div className="space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="bg-white p-6 rounded-lg shadow">
                            <h3 className="text-sm font-medium text-gray-500">Compliance Rate</h3>
                            <p className={`mt-2 text-3xl font-bold ${auditM.complianceRate >= 95 ? 'text-green-600' : 'text-red-600'}`}>{auditM.complianceRate.toFixed(1)}%</p>
                        </div>
                        <div className="bg-white p-6 rounded-lg shadow">
                            <h3 className="text-sm font-medium text-gray-500">Total Overcharge</h3>
                            <p className="mt-2 text-3xl font-bold text-red-600">${auditM.totalOverchargeAmount.toFixed(2)}</p>
                        </div>
                        <div className="bg-white p-6 rounded-lg shadow">
                            <h3 className="text-sm font-medium text-gray-500">Overcharged Claims</h3>
                            <p className="mt-2 text-3xl font-bold text-gray-900">{auditM.overchargeCount.toLocaleString()}</p>
                        </div>
                        <div className="bg-white p-6 rounded-lg shadow">
                            <h3 className="text-sm font-medium text-gray-500">Auditable Records</h3>
                            <p className="mt-2 text-3xl font-bold text-gray-900">{auditM.totalAuditableRecords.toLocaleString()}</p>
                        </div>
                    </div>
                </div>;
            default:
